...
pub fn replace() {
  ...
  fs::write(format!("{}/lead", &*LEAD_ROOT_DIR), include_bytes!("./lead")).expect("Could not update lead");
  fs::write(format!("{}/lead.ps1", &*LEAD_ROOT_DIR), include_bytes!("./lead.ps1")).expect("Could not update lead.ps1");
}
